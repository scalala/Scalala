...
//
///**
// * Marker for being able to get the non-zero values of a map.
// *
// * @author dramage
// */
//trait CanForEachNonZeroDouble[-Coll] {
//  def apply[U](coll : Coll, fn : (Double => U));
//}
//
//object CanForEachNonZeroDouble {
//  type Op[Coll] = CanForEachNonZeroDouble[Coll];
//
//  implicit object OpArrayI extends Op[Array[Int]]
//  { override def apply[U](coll : Array[Int], fn : (Double => U)) =
//    coll.foreach((x : Int) => fn(x.toDouble)); }
//
//  implicit object OpArrayS extends Op[Array[Short]]
//  { override def apply[U](coll : Array[Int], fn : (Double => U)) =
//    coll.foreach((x : Int) => fn(x.toDouble)); }
//
//  implicit object OpArrayL extends Op[Array[Long]]
//  { override def apply(coll : Array[Long], key : Int) = coll(key); }
//
//  implicit object OpArrayF extends Op[Array[Float]]
//  { override def apply(coll : Array[Float], key : Int) = coll(key); }
//
//  implicit object OpArrayD extends Op[Array[Double]]
